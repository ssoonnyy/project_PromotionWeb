...
window.addEventListener("scroll", function () {
  let scrollTarget = document.querySelector("html");
  if (matchMedia("all and (min-width:1024px)").matches) {
    if (scrollTarget.scrollTop >= 80 && scrollTarget.scrollTop < 150) {
      con01Count();
    }
  } else if (
    matchMedia("all and (min-width:801px) and (max-width:1023px)").matches
  ) {
    if (scrollTarget.scrollTop >= 80 && scrollTarget.scrollTop < 110) {
      con01Count();
    }
  } else if (
    matchMedia("all and (min-width:320px) and (max-width:800px)").matches
  ) {
...
function con01Count() {
  ...
  const hybridsCount01 = document.querySelector(".num_box03 > .con01_count01");
